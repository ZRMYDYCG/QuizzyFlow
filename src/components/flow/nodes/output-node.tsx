...
const OutputNode: FC<NodeProps<OutputNodeData>> = ({ data, selected }) => {
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3 mb-3">
          {/* 图标容器 */}
          <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br from-purple-400 to-pink-500 flex items-center justify-center shadow-sm">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500">
              {data.outputType ? getOutputTypeLabel(data.outputType) : '数据流'}
            </span>
          </div>
          {data.format && (
            <div className="flex items-center gap-1.5 ml-1">
              <span className="text-xs text-gray-500 dark:text-gray-400">
                格式: <code className="px-1 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-[10px]">{data.format}</code>
              </span>
            </div>
          )}
        </div>
