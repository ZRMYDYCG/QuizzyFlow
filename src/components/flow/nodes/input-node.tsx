...
const InputNode: FC<NodeProps<InputNodeData>> = ({ data, selected }) => {
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="flex items-start gap-3 mb-3">
          {/* 图标容器 */}
          <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center shadow-sm">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-gray-400 dark:text-gray-500">
              {data.inputType ? getInputTypeLabel(data.inputType) : '数据流'}
            </span>
          </div>
        </div>
      </div>

      {/* Source Handle - 底部输出（输入节点只输出，不接收） */}
      <Handle
        type="source"
